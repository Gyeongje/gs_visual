...
/* ===== Mock Data ===== */
const scheduleSeed = [
  { sat: "TERRA",   start: "10/13 14:41:02", end: "10/13 14:49:10", elev: 62, status: { setup: 12, tracking: 56, processing: 22, delivery: 10 } },
  { sat: "METOP-B", start: "10/13 15:05:28", end: "10/13 15:21:53", elev: 58, status: { setup:  8, tracking: 52, processing: 20, delivery: 12 } },
  { sat: "METOP-C", start: "10/13 15:55:55", end: "10/13 16:09:04", elev: 56, status: { setup: 10, tracking: 48, processing: 18, delivery: 16 } },
  { sat: "TERRA",   start: "10/13 16:17:04", end: "10/13 16:28:43", elev: 52, status: { setup:  9, tracking: 42, processing: 14, delivery: 14 } },
  { sat: "METOP-B", start: "10/13 16:49:15", end: "10/13 17:00:29", elev: 22, status: { setup:  5, tracking: 35, processing: 12, delivery:  8 } },
  { sat: "NOAA 20", start: "10/13 17:34:04", end: "10/13 17:35:34", elev:  5, status: { setup:  2, tracking:  8, processing:  3, delivery:  2 } },
  { sat: "METOP-C", start: "10/13 17:37:10", end: "10/13 17:46:58", elev: 15, status: { setup:  4, tracking: 20, processing:  7, delivery:  5 } },
];
...
const Card = ({ style, children }) => (
  <div style={{ background:"#111114", border:"1px solid #2b2b30", borderRadius:14, padding:12, ...style }}>{children}</div>
);
...
function StatusBar({ parts }) {
  ...
  return (
    <div style={{display:"flex",width:"100%",height:10,background:"#1f1f22",borderRadius:6,overflow:"hidden"}}>
  ...
  return (
    <Card style={{ padding:10 }}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
        <Title>Schedule</Title><Muted>UTC</Muted>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"140px 132px 108px 48px 1fr",color:"#cfcfd4",fontSize:12,fontWeight:600,marginBottom:6}}>
        <div>Satellite</div><div>Start</div><div>End</div><div style={{textAlign:"right"}}>MaxEl</div><div>Status</div>
      </div>
      <div style={{display:"grid",rowGap:6}}>
        {scheduleSeed.map((r)=>(
          <div key={r.sat+r.start} style={{display:"grid",gridTemplateColumns:"140px 132px 108px 48px 1fr",alignItems:"center",columnGap:8}}>
...
function PolarPlot({ height=420 }) {
  const rings=[0.2,0.35,0.5,0.65,0.8,0.95], ticks=[0,45,90,135,180,225,270,315];
  return (
    <Card style={{ padding:10 }}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"260px 1fr",alignItems:"center",gap:14}}>
        ...
        <svg viewBox="0 0 520 520" style={{width:"100%",height}}>
          ...
          {ticks.map(deg=>{
            ...
            return <text key={deg} x={x*520} y={y*520} fill="#a1a1aa" fontSize="12" textAnchor="middle" dominantBaseline="central">{deg}</text>;
          })}
          {polarTargets.map(t=>{
            const r=1-t.el/100; const {x,y}=polarToXY(t.az,r);
            return (
              <g key={t.name}>
                <circle cx={x*520} cy={y*520} r="7" fill="#fde047" />
                <text x={x*520+12} y={y*520-8} fill="#e4e4e7" fontSize="13">{t.name}</text>
              </g>
            );
          })}
          {/* 예시 궤적 */}
  ...
  useEffect(() => {
    const id = setInterval(() => {
      setData(prev => prev.map((s, idx) => {
        ...
        const nextArr = s.points.slice(1); nextArr.push(next);
        power *= decay; if (power < 1) power = 0;
    ...
    <Card style={{ padding:10 }}>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} style={{width:"100%",height:H}}>
        ...
        {[0,0.25,0.5,0.75,1].map((gx,i)=>
          <line key={i} x1={P+gx*(W-2*P)} y1={P} x2={P+gx*(W-2*P)} y2={H-P} stroke="#3f3f46" strokeDasharray="3 3"/>
        )}
...
function EOSFESPanel() {
  const btn = (t)=>(
    <div key={t} style={{padding:"8px 12px",borderRadius:10,background:"#1f1f22",border:"1px solid #3b3b40",color:"#e4e4e7",fontSize:12,textAlign:"center"}}>{t}</div>
  );
  const act = (t)=>(
    <button key={t} style={{padding:"8px 12px",borderRadius:10,background:"rgba(16,185,129,0.18)",border:"1px solid rgba(16,185,129,0.4)",color:"#bbf7d0",fontSize:12}}>
      {t}
    </button>
  );
  return (
    <Card>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
        ...
        <div style={{display:"flex",gap:6}}>
          {["TLEs","Software","Hardware"].map(x=>
            <div key={x} style={{fontSize:11,padding:"2px 8px",borderRadius:8,background:"#15803d",color:"#eafff1"}}>{x}</div>
          )}
        </div>
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:10}}>
        {["Raw","CCSDS","Level 0","PNG","Level 1"].map(x=>(
          <button key={x}
            style={{padding:"10px 8px",borderRadius:12,background:"#efefef14",border:"1px solid #3f3f46",color:"#f2f2f6",fontSize:12}}>
            {x}
          </button>
        ))}
      </div>
...
export default function App() {
  ...
  useEffect(()=>{ const t=setInterval(()=>setNow(fmtTime()),1000); return ()=>clearInterval(t); },[]);

  return (
    <div style={{height:"100vh",width:"100vw",overflow:"hidden",
      background:"linear-gradient(to bottom, #0b2447, #0a0a0b)", color:"#e4e4e7", fontFamily:"Inter, system-ui, sans-serif"}}>
      {/* Header */}
      <div style={{height:46,display:"flex",alignItems:"center",justifyContent:"space-between",
        padding:"0 16px",borderBottom:"1px solid #2b2b30", backdropFilter:"blur(6px)"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div>
            ...
            <div style={{fontSize:11,color:"#a1a1aa",marginTop:-2}}>Site: EOSFES-Primary · User: Orbital Support</div>
          </div>
        </div>
      ...
      {/* Body: 좌 58% / 우 42% */}
      <div style={{display:"grid",gridTemplateColumns:"58% 42%",gap:12, height:"calc(100vh - 46px)", padding:12, boxSizing:"border-box"}}>
